...
const renderHome = (container) => {
    let cachedPostsHtml = sessionStorage.getItem('homePostsCache'); // tenta obter o cache da página no sessionStorage
    
    // verificamos se já existe dados no cache de dados dessa tela
    const postsContent = cachedPostsHtml?cachedPostsHtml:'<div id="posts-container"><h2 style="text-align: center;">Carregando conteúdo...</h2></div>'
    ...
    `;
    if (!cachedPostsHtml) loadPosts(); // só carrega os dados se não houver cache
}

/** Função para buscar e renderizar os artigos do banco */
const loadPosts = async () => {
    ...
    try {
        ...
        sessionStorage.setItem('homePostsCache', stringHtml); // salva no cache do sessionStorage para evitar recarregar do servidor
